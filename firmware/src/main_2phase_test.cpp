...
void detectFall() {
  ...
  float tilt = angleFromVertical(ax, ay, az);
  prevAcc = acc;

  lastAcc  = acc;
  lastTilt = tilt;

  sendDebug(acc, 0, tilt); // Jerk set to 0 for simplified view
  unsigned long now = millis();

  switch (fallState) {
    case IDLE:
      // MODIFIED: We are skipping the Free-Fall check entirely.
      // It now looks directly for the Impact spike.
      if (acc > config.impactThreshold) {
        fallState = IMPACT_SEEN;
        stateEnteredAt = now;
        pushLog("TestMode: Impact detected! Checking tilt...");
      }
      break;

    case IMPACT_SEEN:
      // MODIFIED: We only wait 500ms to let the sensor settle, then check Tilt.
      if (now - stateEnteredAt > 500) { 
        bool isTilted = (tilt > config.tiltThreshold);

        if (isTilted) {
          fallState = CONFIRMING;
          stateEnteredAt = now;
          pushLog("TestMode: FALL CONFIRMED (Impact + Tilt)");
          ...
          fallState = IDLE;
          pushLog("TestMode: Rejected (Not tilted enough)");
        }
      }
      break;

    case CONFIRMING:
      if (now - stateEnteredAt > 10000) { // Reduced cooldown to 10s for faster testing
        fallState = IDLE;
        pushLog("TestMode: Re-armed");
      }
      break;
  }
}
