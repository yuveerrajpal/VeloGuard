// VeloGuard — 4-Phase Production Firmware
// Full FSM: IDLE → FREE_FALL → IMPACT_SEEN → CONFIRMING
// Includes free-fall detection, impact threshold, post-impact stillness + tilt confirmation.
// GPS-linked SMS alert sent to up to 3 contacts on confirmed crash.
...
void setup() {
  ...
  if (config.magic != CONFIG_MAGIC) {
    ...
    config.impactThreshold = 25.0f;
    config.jerkThreshold   = 3.0f;
    config.tiltThreshold   = 50.0f;
    config.testMode        = 1;
    saveConfig();
  ...
  // MPU6050 — ±16G range avoids sensor saturation at crash-speed impacts
  Wire.begin(15, 16);
  if (!mpu.begin()) {
    ...
    pushLog("MPU6050: started — range 16G, filter 21Hz");
  }
...
void detectFall() {
  ...
  switch (fallState) {
    ...
    // jerkThreshold is repurposed as the free-fall floor (default 3.0 m/s²)
    case IDLE:
    ...
    // If no spike within 700ms, it was just a bump or road dip — re-arm.
    case FREE_FALL:
    ...
    // Collect 2.5s of post-impact data. Real fall = tilted and stops moving.
    case IMPACT_SEEN:
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <div class="row4">
      <div><label>Impact Threshold (m/s²)</label><input id="impact" type="number" step="0.5"><p class="hint">Spike needed after free-fall. Default: 25</p></div>
      <div><label>Free-Fall Floor (m/s²)</label><input id="jerk" type="number" step="0.1"><p class="hint">Accel drop = weightless. Default: 3.0</p></div>
      <div><label>Tilt Threshold (°)</label><input id="tilt" type="number" step="1"><p class="hint">Angle from upright. Default: 50</p></div>
      <div><label>Test Mode (1=ON, 0=LIVE)</label><input id="testMode" type="number" min="0" max="1" step="1"><p class="hint">ON: streams telemetry, enables SIM tests.</p></div>
    </div>
  </div>
  <div class="card">
...
async function runSimChecks() {
  ...
  try {
    ...
    document.getElementById("simOut").innerText =
      "Network : " + j.network + "\n" + "SIM     : " + j.sim + "\n" + "SMS     : " + j.sms + "\n" + "GPS     : " + j.gps;
  } catch { document.getElementById("simOut").innerText = t; }
}
loadConfig();
</script>
</body>
